...
function MakeFont($fontfile, $enc='cp1252', $embed=true,$dir_file_save='')
{
	...
	if($embed)
	{
		...
		{
			$file = $basename.'.z';
			SaveToFile($dir_file_save.$file, gzcompress($info['Data']), 'b');
			$info['File'] = $file;
	...
	MakeDefinitionFile($dir_file_save.$basename.'.php', $type, $enc, $embed, $map, $info);
...
if(PHP_SAPI=='cli')
{
	...
	if($argc>=4)
		...
		$embed = true;
	MakeFont($fontfile, $enc, $embed,$dir_file_save);
}
?>
